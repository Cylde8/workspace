...
st.title("App de predicción del tiempo")
st.write("Consigue la predicción del tiempo de cualquier lugar con Open-Meteo API.")
st.write("https://open-meteo.com/en/docs")
st.write("https://streamlit.io/: Para crear webs interactivas")
...
with col1:
    lat = st.number_input("Introduce la latitud:", value=0.0, format="%.6f", step=0.000001)
with col2:
    lon = st.number_input("Introduce la longitud:", value=0.0, format="%.6f", step=0.000001)

fetch_button = st.button("Consigue la predicción")


# STEP6: Validamos lo que introduce el usuario
if lat < -90 or lat > 90:
    st.error("La latitud debe estar entre -90 y 90.")
...
if fetch_button and (-90 <= lat <= 90) and (-180 <= lon <= 180):
    ...
    if data:
        ...
        st.write(f"Predicción del tiempo para Latitud: {lat}, Longitud: {lon}")
        ...
        fig = px.line(forecast_df, x='Tiempo', y='Temperatura (°C)', title='Predicción de temperatura')
